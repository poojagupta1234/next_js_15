...
import ShopingCart from "@/app/components/ecom/shoppingCart/shopingCart";
import Link from 'next/link';


export default function Header() {
    
      const { user, token, logout , totalQuantity, totalPrice, toggleDrawer} = useAuth();
      const router = useRouter();
    // Get the token from user or set to null if not available
    ...
    const handleNavigationClick = (): void => {
             router.push(`/blink/dashboard/${user?.role}`); // Redirect to admin dashboard
        
    };

    const handleLoginClick = (loginType: string): void => {
        // Handle login logic here
        if(loginType === "login") {
        router.replace("/blink/login"); // Redirect to login page
        }
        else {
            // If the user is already logged in, you might want to handle logout logic here
                console.log("Logging out...");
           logout();
             router.push("/blink"); // Redirect to login page
    ...
    return (
        <header className={HeaderCss.header}>
            ...
            <div className={HeaderCss.logoBar}>
                ...
                <address className={HeaderCss.address}>
                    <h3 className={HeaderCss.h3}>Delivery in 8 minutes</h3>
                    <p className={HeaderCss.para}>
                        Springfield, IL 62701 217-555-5555
                    </p>
                </address>
            </div>
            ...
            <div className={HeaderCss.headerButton}>
              
                     {token && ( 
                          <button className={HeaderCss.user} onClick={handleNavigationClick}>
                            <FaUser /> {user?.role === "admin" ? "Admin" : "User"}
                            </button>
                         )}
                
                <button className={HeaderCss.login} onClick={()=> handleLoginClick(token ? "logout" : "login")}>{token ? "Logout" : "Login"}</button>
                <button className={HeaderCss.mycart} onClick={handleCartClick}>
                    <FaCartPlus />
                    <div >
